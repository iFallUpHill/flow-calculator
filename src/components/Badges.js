  ...
  return (
    <div className="flex gap-1">
      <p className="text-xs mb-2 font-semibold inline-block py-1 px-2 rounded-full text-black transition-colors duration-150 bg-zinc-100 hover:bg-zinc-200 cursor-pointer">
        <a
          href="https://github.com/iFallUpHill/flow-calculator"
          target="_blank"
...
const AdvancedBadge = ({label}) => {
  return (
    <span className="text-xs font-semibold inline-block py-1 px-2 rounded-md text-red-600 bg-red-200 uppercase">
      {label}
    </span>
  )
};
...
const WarningBadge = ({label}) => {
  return (
    <span className="text-xs font-semibold inline-block py-1 px-2 rounded-md text-amber-600 bg-amber-200 uppercase">
      {label}
    </span>
  )
};
...
export { TitleBadges, AdvancedBadge, WarningBadge, LabelBadge };
